...
impl AetherDataset {
    ...
    /// ```
    pub fn map_column<F>(&self, column: &str, output: &str, f: F) -> Result<Self, DatasetError>
    where
        F: Fn(f64) -> f64,
    {
        let idx = self
...
fn cast_column_to_f64(arr: &Arc<dyn Array>) -> Result<Arc<dyn Array>, arrow::error::ArrowError> {
...
mod tests {
    ...
    fn test_map_column_add_new() {
        let ds = make_test_dataset();
        let mapped = ds
            .map_column("p_value", "neg_log_p", |x| -x.log10())
            .unwrap();
