...
│  L3: select / sort / limit / union                       │
│  L4: summarize / ols / ivw / egger                       │
└────────────────────────┬────────────────────────────────┘
                         │ 调用
┌────────────────────────▼────────────────────────────────┐
│  DatasetStore   (data-engine/src/dataset/store.rs)       │
│  · 命名注册表  HashMap<String, Arc<Dataset>>              │
│  · 内部 SessionContext（in-memory only）                 │
│  · map_expr / sql_query                                  │
└────────────────────────┬────────────────────────────────┘
                         │ 持有
┌────────────────────────▼────────────────────────────────┐
...
## 4. DatasetStore —— Agent 工作内存（纯 in-memory）

命名注册表。内部持有私有 `SessionContext`，**只**用来执行已注册 in-memory 数据集上的 SQL（`map_expr` / `sql_query`）；不携带任何外部 catalog —— 與 Iceberg 的交互由 [`DataSession`](#5-datasession--iceberg--session--workspace) 負責。

### 注册表 CRUD
`new()` · `put(ds)` · `put_overwrite(ds) → Arc<Dataset>` · `get(name)` · `drop(name)` ·
`list()` · `exists(name)`

### In-memory SQL
- `sql_query(sql)` — in-memory SQL → 临时 `Dataset`（不入注册表）
- `map_expr(name, source, column, expr, output_col)` — DataFusion 表达式变换列（可替换或新增列）

### 内部 helper
- `register_all_as_tables()` / `register_as_table(ds)` — 把 `Dataset` 注册为 DataFusion `MemTable`
- `insert_dataset(ds)` / `collect_df(...)` — DataFusion 计划 → `Dataset`，入注册表

## 5. DataSession — Iceberg / Session / Workspace

`DataSession` 持有 Iceberg-aware `SessionContext`（`"iceberg"` catalog 已注册），负责所有 Iceberg ↔ in-memory 边界的桥接。

### Iceberg 摄入 / 检视
- `read_table(&store, name, ns, tbl, cols?, filter?, limit?)` — **Iceberg 表 → 注册到 `store`**
- `peek_table(ns, tbl, cols?, filter?, limit?)` — **Iceberg 表 → 临时 `Dataset`（不入注册表）**

### Catalog / SQL
- `list_tables()` / `list_tables_in_namespace(ns)` / `describe_table(ns, tbl)`
- `sql_query(sql)` — 任意 SQL（含 `iceberg."ns"."tbl"` 与 in-memory 表）

### 内部 helper
- `build_table_sql(ns, tbl, cols, filter, limit)` — 构造 `SELECT … FROM iceberg."ns"."table"`（必须用点号 3 段名 `catalog.schema.table`，逗号形式会被 SQL 当成 cross join）

## 6. Agent 工具层（iceberg-tools/）

每个工具遵循 `agentik-core` 的 `ToolFunction` 模式：`#[derive(ToolInput)]` 输入结构 + 持有 `Arc<DataSession>` / `Arc<DatasetStore>` 的工具结构。

`dataset_load_table` 是**唯一**同時持有 `Arc<DataSession>` 與 `Arc<DatasetStore>` 的工具（透過 `workspace.read_table(&store, ...)` 註冊到 store）。其他 13 個 dataset 工具僅持有 `Arc<DatasetStore>`。
...
| 工具 | 封装 | 对 store | 状态 |
|---|---|---|---|
| `dataset_load_table` | `workspace.read_table(&store, ...)` | 注册新数据集（L1） | ✅ |
...
| **L3 变换** | select / sort / limit / union / map | ✅ |
...
`DataSession` 与 `DatasetStore` 在 agent 启动时构造，两者独立：

```rust
// crates/agentik-runtime/src/tools.rs
let workspace = Arc::new(DataSession::new().await?);
...
6. **职责分离：`DatasetStore` vs `DataSession`** —— `DatasetStore` 僅負責 in-memory 命名註冊與 in-memory SQL；與 Iceberg 的所有交互（`read_table` / `peek_table`、catalog 發現、跨源 SQL）統一歸 `DataSession`。

## 10. 文件清单

| 文件 | 内容 |
|---|---|
| `crates/data-engine/src/dataset.rs` | `Dataset` 核心模型 |
| `crates/data-engine/src/dataset/store.rs` | `DatasetStore` 注册表（纯 in-memory）|
| `crates/data-engine/src/data_session.rs` | `DataSession`（Iceberg-aware `SessionContext`、`read_table` / `peek_table`）|
| `crates/data-engine/src/error.rs` | `DatasetError` |
| `crates/iceberg-tools/src/dataset/mod.rs` | 工具模块 + `dataset_registrations`（L1–L4 全部 14 个工具）|
| `crates/iceberg-tools/src/dataset/load_table.rs` | `dataset_load_table`（L1，唯一同時持有 `DataSession` + `DatasetStore`）|
...
- `Dataset` 单测：构造、列提取、空值策略、变换、血缘（25 个）
- `DatasetStore` 单测：CRUD、in-memory SQL、map_expr（10 个）
- `DataSession` 单测：`build_table_sql`（1 个）
- `iceberg-tools` common 单测（6 个）
- 集成测试（需 Iceberg REST）：2 个（默認 ignored）
