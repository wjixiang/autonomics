...
async fn test_client_creation_with_api_key() {
    let client =
        Anthropic::new("test-api-key", "https://api.anthropic.com").expect("Should create client");
...
async fn test_config_with_invalid_url() {
    let config =
        ClientConfig::new("test-key", "https://api.anthropic.com").with_base_url("invalid-url");
    let result = config.validate();
    assert!(result.is_err());
    assert!(
        result
