...
```
┌─────────────────────────────────────────────────────────┐
│  Agent Tools (aether-tools/src/dataset/)                 │
...
**Iceberg 工具**（`aether-tools/src/`，操作 Iceberg catalog，前缀 `iceberg_`）:
...
let mut tools = aether_tools::aether_registrations(workspace);
...
| `crates/aether-tools/src/dataset/mod.rs` | 工具模块 + `dataset_registrations`（L1–L4 全部 15 个工具）|
| `crates/aether-tools/src/dataset/load_table.rs` | `dataset_load_table`（L1）|
| `crates/aether-tools/src/dataset/list.rs` | `dataset_list`（L2）|
| `crates/aether-tools/src/dataset/describe.rs` | `dataset_describe`（L2）|
| `crates/aether-tools/src/dataset/preview.rs` | `dataset_preview`（L2）|
| `crates/aether-tools/src/dataset/drop.rs` | `dataset_drop`（L2）|
| `crates/aether-tools/src/dataset/select.rs` | `dataset_select`（L3）|
| `crates/aether-tools/src/dataset/sort.rs` | `dataset_sort`（L3）|
| `crates/aether-tools/src/dataset/limit.rs` | `dataset_limit`（L3）|
| `crates/aether-tools/src/dataset/union.rs` | `dataset_union`（L3）|
| `crates/aether-tools/src/dataset/sql.rs` | `dataset_sql`（L3）|
| `crates/aether-tools/src/dataset/map.rs` | `dataset_map`（L3）|
| `crates/aether-tools/src/dataset/summarize.rs` | `dataset_summarize`（L4）|
| `crates/aether-tools/src/dataset/ols.rs` | `dataset_ols`（L4）|
| `crates/aether-tools/src/dataset/ivw.rs` | `dataset_ivw`（L4）|
| `crates/aether-tools/src/dataset/egger.rs` | `dataset_egger`（L4）|

## 10. 测试

- `AetherDataset` 单测：构造、列提取、空值策略、变换、血缘（25 个）
- `DatasetStore` 单测：CRUD、SQL 变换、跨数据集查询（8 个）
- 当前共 **54 个测试全绿**（33 AetherDataset + 11 DatasetStore + 6 aether-tools common + 4 integration）
